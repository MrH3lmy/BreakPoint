...
def _parse_channel(packet: RadioTap) -> str:
...
def _parse_ssid(packet: RadioTap) -> str:
...
def _parse_rssi(packet: RadioTap) -> str:
...
def _print_table(access_points: Dict[str, AccessPoint]) -> None:
    ...
    rows = sorted(access_points.values(), key=lambda ap: (_signal_value(ap), ap.bssid), reverse=True)
...
def main() -> int:
    ...
    def _handle_packet(packet: RadioTap) -> None:
        ...
        access_points[bssid] = AccessPoint(
            bssid=bssid,
            ssid=_parse_ssid(packet),
            rssi=_parse_rssi(packet),
            channel=_parse_channel(packet),
        )
